...
export async function listPumps(ctx: MyContext) {
	...
	const tokenAddrPromises = await Promise.all(
		active_pumps.map(async (ca) => {
	...
	try {
		...
		const message = await Promise.all(
			infos.map(async (data, index) => {
				...
				const percent = bonding_curve?.bonding_percent
					? bonding_curve.bonding_percent
					: "idk Maybe";
				const progress = bonding_curve?.progress_bar
