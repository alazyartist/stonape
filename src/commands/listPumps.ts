	...
	try {
		...
		const message = await Promise.all(infos
			.map(async (data, index) => {
				...
				const percent = bonding_curve?.bonding_percent ?bonding_curve.bonding_percent :'idk Maybe';
				const progress = bonding_curve?.progress_bar ?bonding_curve?.progress_bar:'🟩🟩🟩🟩🟩🟩🟩⬜⬜⬜';
